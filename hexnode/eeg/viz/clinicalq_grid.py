#!/usr/bin/env python3
"""
Clinical Q Grid Generator (adapted from topo_sheet_generator.py)

Generates RyHa-style multi-panel topomap sheets (3x5, 5x5, etc.) for EEG analysis.
Matrix layouts: rows = metrics (Absolute, Relative, Z-Score), cols = bands.
Outputs topo_sheet_*.png for gallery integration.

Source: EEG_Paradox_Decoder/standalone_traceroute_netops/cracker/visualization/topo_sheet_generator.py
"""
...
from hexnode.eeg.viz.band_definitions import get_band_frequency_range_str
...
try:
    ...
    _HAS_TOPOMAP_GENERATOR = True
except ImportError:
    TopomapGenerator = None
    _HAS_TOPOMAP_GENERATOR = False
...
def get_band_frequency_range(band: str, definitions=None) -> str:
    """Thin wrapper matching the original topomap_generator API."""
    return get_band_frequency_range_str(band, definitions)


class _StubTopomapGenerator:
    """Minimal fallback when TopomapGenerator is unavailable."""

    def __init__(self, config=None):
        self.config = config

    def plot_topomap_into_axes(self, ax, values, channel_names, title,
                               is_zscore=False, allow_constant=True) -> bool:
        if ax is not None:
            ax.axis('off')
            try:
                fg = get_theme_manager().get_foreground_color()
            except Exception:
                fg = '#d6f6ff'
            ax.set_title(f"{title}\n(topomap N/A)", color=fg, fontsize=9)
        return False


class ClinicalQGridGenerator:
    """Generates multi-panel topomap sheets (clinical QEEG z-score grids)."""

    def __init__(self, config=None):
        self.config = config or get_visualization_config()
        self.theme = get_theme_manager()
        if _HAS_TOPOMAP_GENERATOR:
            self._topomap_gen = TopomapGenerator(config)
        else:
            self._topomap_gen = _StubTopomapGenerator(config)
            logger.warning("TopomapGenerator not available; topomap cells will be placeholders")
        self._sheet_bands_summary = self.config.get(
            'topomaps.topo_sheets.summary_bands', SHEET_BANDS_5
        )
    ...
    def _compute_relative_power(
        self, metrics_by_site: Dict[str, Any], band: str,
        epoch: Optional[str], bands_all: List[str]
    ) -> Tuple[List[str], Any]:
    ...
    def _zscore_values(self, values) -> Any:
    ...
    def _composite_sheet_from_existing_images(
        self, viz_dir: Path, file_pattern: str, bands: List[str],
        epochs: List[Optional[str]], out_filename: str, key_prefix: str
    ) -> Dict[str, Path]:
        paths = {}
        for ep in epochs:
            ...
            try:
                ...
                img_objs = [
                    Image.open(next(p for x, p in imgs if x == b)).convert('RGB')
                    for b in band_order
                ]
                if len(img_objs) < 2:
                    continue
    ...
    def generate_zscore_summary_sheet(
        ...
        """
        3x5 grid: rows = Absolute Power Z, Relative Power Z, Amplitude Asymmetry;
        ...
        if not epochs:
            epochs = [None]
        bands_all_lower = [b.lower() for b in ['Delta', 'Theta', 'Alpha', 'Beta', 'Gamma', 'SMR', 'HiBeta']]
        ...
        for epoch in epochs:
            ...
            fig.text(
                0.02, 0.98, "Clinical Q Grid - Z-Score Summary Sheet",
                fontsize=11, fontweight='bold', color=self.theme.NEON_CYAN,
                ha='left', va='top',
                bbox=dict(
            ...
            for row_idx, row_label in enumerate(row_labels):
                for col_idx, band in enumerate(bands):
                    ...
                    freq = get_band_frequency_range(band)
                    ...
                    try:
                        if row_idx == 0:
                            ...
                            if pairs:
                                ...
                                plot_connectivity_cell(
                                    ax, pairs, title, mode='asymmetry',
                                    theme=self.theme, options=opts if opts else None
                                )
                            else:
                                ax.axis('off')
                                ax.set_title(title, fontsize=10, color=self.theme.get_foreground_color())
            ...
            fig.savefig(
                filepath, format='png',
                dpi=self.config.get('default_dpi', 150),
                facecolor=self.theme.get_background_color(),
    ...
    def generate_zscore_extended_sheet(
        ...
        """
        5x5 grid: rows = Absolute Power Z, Relative Power Z, Amplitude Asymmetry,
        Coherence, Phase Lag; cols = Delta, Theta, Alpha, Beta, HiBeta.
        """
        if not self.config.are_topomaps_enabled() or not self.config.get('topomaps.generate_z_scores', True):
            return {}
        output_dir.mkdir(parents=True, exist_ok=True)
        ...
        for epoch in epochs:
            ...
            fig, axes = plt.subplots(
                n_rows, n_cols, figsize=(3.5 * n_cols, 2.8 * n_rows),
                facecolor=self.theme.get_background_color()
            )
            fig.patch.set_facecolor(self.theme.get_background_color())
            fig.text(
                0.02, 0.98, "Clinical Q Grid - Z-Score Extended (5x5)",
                fontsize=11, fontweight='bold', color=self.theme.NEON_CYAN,
                ha='left', va='top',
                bbox=dict(
                    boxstyle='round,pad=0.3',
            ...
            row_labels = [
                'Absolute Power Z', 'Relative Power Z',
                'Amplitude Asymmetry', 'Coherence', 'Phase Lag'
            ]
            for row_idx, row_label in enumerate(row_labels):
                for col_idx, band in enumerate(bands):
                    ...
                    try:
                        if row_idx == 0:
                            ...
                            if coh_pairs:
                                opts = self.config.get('connectivity', {}) or {}
                                plot_connectivity_cell(
                                    ax, coh_pairs, title, mode='coherence',
                                    theme=self.theme, options=opts if opts else None
                                )
                            else:
                                ax.axis('off')
                            ...
                            if phase_pairs:
                                opts = self.config.get('connectivity', {}) or {}
                                plot_connectivity_cell(
                                    ax, phase_pairs, title, mode='phase',
                                    theme=self.theme, options=opts if opts else None
                                )
                            else:
                                ax.axis('off')
    ...
    def generate_zscore_power_ratio_sheet(
        ...
        for epoch in epochs:
            ...
            fig.patch.set_facecolor(self.theme.get_background_color())
            fig.text(
                0.02, 0.98, "Clinical Q Grid - Z-Score Power Ratio Sheet",
                fontsize=11, fontweight='bold', color=self.theme.NEON_CYAN,
                ha='left', va='top',
                bbox=dict(
                    boxstyle='round,pad=0.3',
            ...
            for idx, (b1, b2) in enumerate(ratio_pairs):
                ...
                f1, f2 = get_band_frequency_range(b1), get_band_frequency_range(b2)
    ...
    # ------------------------------------------------------------------
    # Band Power / Absolute / Relative / Diff sheets
    # ------------------------------------------------------------------
    ...
    def generate_bandpower_sheet(
        ...
        for epoch in epochs:
            n_cols = len(bands)
            fig, axes = plt.subplots(
                1, n_cols, figsize=(3.2 * n_cols, 3.5),
                facecolor=self.theme.get_background_color()
            )
            fig.patch.set_facecolor(self.theme.get_background_color())
            fig.text(
                0.02, 0.98, "Clinical Q Grid - Band Power Sheet",
                fontsize=11, fontweight='bold', color=self.theme.NEON_CYAN,
                ha='left', va='top',
                bbox=dict(
                    boxstyle='round,pad=0.3',
            ...
            for col_idx, band in enumerate(bands):
                ...
                band_part = f"{band_display} ({freq})" if freq else band_display
                title = f"{band_part}\nBand Power"
                self._plot_topomap_cell(ax=axes[col_idx], values=np.array(values),
                                        channel_names=ch_names, title=title, is_zscore=False)
            ...
            plt.close(fig)
            paths[f"sheet_bandpower{suffix}"] = filepath
    ...
    def generate_absolute_sheet(
        ...
        for epoch in epochs:
            ...
            fig.patch.set_facecolor(self.theme.get_background_color())
            fig.text(
                0.02, 0.98, "Clinical Q Grid - Absolute Power Sheet",
                fontsize=11, fontweight='bold', color=self.theme.NEON_CYAN,
                ha='left', va='top',
                bbox=dict(
                    boxstyle='round,pad=0.3',
    ...
    def generate_relative_sheet(
        ...
        for epoch in epochs:
            ...
            fig.patch.set_facecolor(self.theme.get_background_color())
            fig.text(
                0.02, 0.98, "Clinical Q Grid - Relative Power Sheet",
                fontsize=11, fontweight='bold', color=self.theme.NEON_CYAN,
                ha='left', va='top',
                bbox=dict(
                    boxstyle='round,pad=0.3',
    ...
    def generate_diff_sheet(
        ...
        """EO minus EC difference sheet (requires both conditions)."""
        if not self.config.are_topomaps_enabled():
            return {}
        is_nested = is_nested_structure(metrics_by_site)
        ...
        fig.patch.set_facecolor(self.theme.get_background_color())
        fig.text(
            0.02, 0.98, "Clinical Q Grid - Difference Sheet (EO - EC)",
            fontsize=11, fontweight='bold', color=self.theme.NEON_CYAN,
            ha='left', va='top',
            bbox=dict(
                boxstyle='round,pad=0.3',
    ...
    def generate_all_sheets(
        ...
        """Generate all clinical Q grid topo sheets. Returns dict of key -> Path."""
        ...
        def _composite_fallback(pattern, bands, out_name, key_prefix):
        ...
        if self.config.get('topomaps.topo_sheets.enabled', True):
            try:
                if self.config.get('topomaps.topo_sheets.zscore_summary', True) and self.config.get('topomaps.generate_z_scores', True):
                    ...
                    if not p and len(bands_5) >= 2:
                        p = _composite_fallback('topomap_zscore_{band}', bands_5,
                                                'topo_sheet_zscore_summary_3x5', 'sheet_zscore_summary')
                    all_paths.update(p)
            except Exception as e:
            ...
            try:
                if self.config.get('topomaps.topo_sheets.zscore_extended', True) and self.config.get('topomaps.generate_z_scores', True):
                    ...
                    if not p and len(bands_5) >= 2:
                        p = _composite_fallback('topomap_zscore_{band}', bands_5,
                                                'topo_sheet_zscore_extended_5x5', 'sheet_zscore_extended')
                    all_paths.update(p)
            except Exception as e:
            ...
            try:
                if self.config.get('topomaps.topo_sheets.bandpower', True):
                    ...
                    if not p and len(bands_all) >= 2:
                        p = _composite_fallback('topomap_{band}', bands_all,
                                                'topo_sheet_bandpower', 'sheet_bandpower')
                    all_paths.update(p)
            except Exception as e:
            ...
            try:
                if self.config.get('topomaps.topo_sheets.absolute', True):
                    ...
                    if not p and len(bands_all) >= 2:
                        p = _composite_fallback('topomap_{band}', bands_all,
                                                'topo_sheet_absolute', 'sheet_absolute')
                    all_paths.update(p)
            except Exception as e:
            ...
            try:
                if self.config.get('topomaps.topo_sheets.relative', True):
                    ...
                    if not p and len(bands_all) >= 2:
                        p = _composite_fallback('topomap_relative_{band}', bands_all,
                                                'topo_sheet_relative', 'sheet_relative')
                    all_paths.update(p)
            except Exception as e:
...
TopoSheetGenerator = ClinicalQGridGenerator
